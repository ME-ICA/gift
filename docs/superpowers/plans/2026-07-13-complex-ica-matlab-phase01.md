...
Zm = Z(mask, :).';                    % (T x Vm)
% Per-VOXEL temporal de-mean: removes the static complex baseline image. (Deliberate axis
% choice for complex data -- see reference doc §5 -- leaving a clean mixture
% Zm = tc_dm * Strue, with Strue(k,v) = Smaps(k,v)*exp(1i*phi(v)).)
Zm = Zm - mean(Zm, 1);

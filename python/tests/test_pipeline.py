...
def test_back_reconstruction_is_subject_specific():
    ...
    for _ in range(n_sub):
        ...
        Strue_i.append(
            np.concatenate(
                [Si * np.exp(1j * phi)[None, :],
                 np.zeros((N, Vnoise), dtype=np.complex128)],
                axis=1,
            )
        )
    ...
    res = run_complex_ica(subjects, n_components=N, n_subject=6, estimator="cebm",
                          rng=np.random.default_rng(0))

    for (S_i, _A_i), St in zip(res.subjects, Strue_i):
        St_m = St[:, res.mask]

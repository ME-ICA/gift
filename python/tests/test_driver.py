...
from gift.complex_io import read_complex
from gift.driver import run_from_files, unmask
...
def _write_subject(tmp_path, name, rng, Smaps, phi, T, dims):
    ...
    nb.save(nb.Nifti1Image(vol.real.astype(np.float64), np.eye(4)), str(r))
    nb.save(nb.Nifti1Image(vol.imag.astype(np.float64), np.eye(4)), str(i))
    return (r, i)
...
def test_run_from_files_end_to_end(tmp_path):
    ...
    pairs = [
        _write_subject(tmp_path, f'sub{k:02d}', rng, Smaps, phi, T, dims) for k in range(n_sub)
    ]
    ...
    expected = unmask(res.S_group, res.mask, dims)  # (N, *dims)
    for k, (first, second) in enumerate(written):
        assert first.exists()
        assert second.exists()
        z = read_complex(real_file=first, imag_file=second)
        assert z.shape == dims
        ...
        # the written pair round-trips to the in-memory map it came from, which also
        # pins the R_/I_ files to the real&imaginary reading rather than mag&phase
        assert np.allclose(z, expected[k])
